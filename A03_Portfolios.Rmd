...
### Exercise 1 Preparation: Get the data and calculate monthly simple returns

Sebastian and I selected the following 10 stocks from the S&P 500:
...
We create a vector with the Stocks and then get the data with the tq_get function

```{r get Data and caluclate stock prices}
stockselection <- c("AXP", "AAPL", "BAC", "BK", "C", "JPM", "PEP", "ORCL", "NKE", "LUV")
stocks.prices <- stockselection %>%
  tq_get(get  = "stock.prices", from = "2000-01-01",to = "2018-07-31") %>%
  dplyr::group_by(symbol)
stocks.prices
```
```{r create monthly return for the 10 stocks. Make 10 columns for each stock one column with covmatrix}
...
Proofing that the stocks are stationary with a plot
Indication:


 * no noticeable trends and changing levels 
 
 * we can't observe a trend in variance
 
 * no prominent seasonality can be observed in series
 
```{r}
Plot<- ggplot(stocks.returns.monthly, aes(date, Stock.returns, color= symbol))+
...
Then we create a timeseries -> xts file
...
```
### Exercise 1a: Steps before starting with the analysis
...
### Exercise 1b: Get the data and calculate monthly simple returns
Plot the average return of the assets against their standard deviation. Are there any dominated assets? Plot the efficient frontier using `chart.EfficientFrontier` and `chart.EF.Weights` (also check the `demo(demo_efficient_frontier)` from the `portfolioAnalytics`-package.
...
Bringing mu and Sigma together and create sigmamuggplot
```{r}
...
sigmamuggplot <- ggplot(sigmamu, aes(sigma, mu))+
 ...
 sigmamuggplot 
```
We can see that no assets dominates all the other assets. Therefore no asset is efficient. 

### Exercise 1c Part 1: Portfolio Analysis
Now comes the fun: Work through the vignette of the `portfolioAnalytics`-package
(`vignette("portfolio_vignette")`), set a full investment constraint and limit the portfolio weights to be 'long only' and calculate minimum-variance/maximum-return and quadratic utility portfolios.

long only portfolio & full investment
```{r}
...
portfull <- add.constraint(portfolio=portfull, type="long_only")
...
chart.EF.Weights(prt_ef_full, colorset=rainbow(n = length(stockselection)), match.col="StdDev", cex.lab = 1.5, main = "StdDev")
```
Minimum varriance
```{r}
...
plot(opt_maxret, risk.col="StdDev", return.col="mean",
       ...
       xlim=c(0, 0.2), ylim=c(0,0.03))
```
Calculate quadratic utility portfolio
```{r}
qu <- add.objective(portfolio=port_l, type="return", name="mean")
qu <- add.objective(portfolio=qu, type="quadratic_utility", name="var", risk_aversion=4)
...
plot(opt_qu, risk.col="StdDev", return.col="mean",
      ...
      xlim=c(0, 0.15), ylim=c(0, 0.030))
```
### Exercise 1c Part 2: Portfolio Analysis
c)	Allow for short selling (delete the long only constraint). What happens to your portfolio? Illustrate using the efficient frontier! Combine efficient frontiers using `chart.EfficientFrontierOverlay` to highlight the differences.


short selling & full_invest

```{r}
portshort <- portfolio.spec(assets = colnames(stock.returns.timeseries.xts))
portshort <- add.constraint(portfolio=portshort, type="full_investment",)
meanvar.portf.short <- add.objective(portfolio=portshort, type="return", name="mean")
meanvar.portf.short <- add.objective(portfolio=portshort, type="risk", name="StdDev")
prt_ef_full <- create.EfficientFrontier(R=stock.returns.timeseries.xts, portfolio=portshort, type="mean-StdDev", match.col = "StdDev")
chart.EfficientFrontier(prt_ef_full, match.col="StdDev", type="b", rf=NULL, pch.assets = 1)
chart.EF.Weights(prt_ef_full, colorset=rainbow(n = length(stockselection)), match.col="StdDev", cex.lab = 1.5, main = "StdDev")
```

### Exercise 1d: Portfolio Analysis
Play around with the constraints and see what happens. Illustrate using `chart.EfficientFrontierOverlay`.
Here is a diversification portfolio

```{r}

port <- portfolio.spec(assets = colnames(stock.returns.timeseries.xts),
                       category_labels = stockselection)
port_c <- add.constraint(portfolio=port, type="box", min=-0.1, max=0.15)
meanvar.portf <- add.objective(portfolio=port_c, type="return", name="mean")
meanvar.portf <- add.objective(portfolio=port_c, type="risk", name="StdDev")
summary(meanvar.portf, digits=3)
prt_ef <- create.EfficientFrontier(R=stock.returns.timeseries.xts, portfolio=port_c, type="mean-StdDev", match.col = "StdDev")
...
```{r}
portf.list.c <- combine.portfolios(list(portfull, portshort, port_c))
legend.labels <- c("Full Investment", "Short", "Constraints")
chart.EfficientFrontierOverlay(R=stock.returns.timeseries.xts,
                               ...
                               legend.labels=legend.labels, cex.legend=1,
