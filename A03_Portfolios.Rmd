...
# Solution Exercise 1
My personal dataset of 10 stocks
```{r}
stockselection <- c("AAPL", "AMZN", "MSFT", "JPM", "WMT", "CSCO", "XOM", "BAC", "NKE", "PFE")
...
stocks.prices
```
Create monthly retunrs for the 10 stocks
```{r}
...
Calculating the Mu
```{r}
stock.returns.timeseries.mu.xts <- lapply (stock.returns.timeseries_xts, FUN = mean)
stock.returns.timeseries.mu.xts
```
Calculating the Sigma (Standard deviation)
```{r}
stock.returns.timeseries.sigma.xts <- lapply(stock.returns.timeseries_xts, FUN = sd)
...
cormatrix
```
Plot the correlations (causal relationship (hair & planet))
```{r}
chart.Correlation(R = stock.returns.timeseries_xts, use = "everything", method = "pearson")
```
Calculate the covariance matrix
```{r}
covmatrix <- cov(stock.returns.timeseries_xts, use = "everything", method = "pearson")
covmatrix
```
...
sigmamuggplot <- ggplot(sigmamu, aes(sigma, mu))+
  ...
  geom_label_repel(aes(label = symbol),
                  ...
                  point.padding = 0.1,
                  label.size = 0.2,
                  segment.color = 'grey50', size = 2.5)+                      theme_classic()
...
prt_ef <- create.EfficientFrontier(R=stock.returns.timeseries_xts, portfolio=port, type="mean-StdDev", match.col = "StdDev")
...
```
full investment (the same just for the full investment)
```{r}
...
prt_ef_full <- create.EfficientFrontier(R=stock.returns.timeseries_xts, portfolio=portfull, type="mean-StdDev", match.col = "StdDev")
...
Minimum varriance
```{r}
port_l <- portfolio.spec(assets = colnames(stock.returns.timeseries_xts))
port_l <- add.constraint(portfolio = port_l,
type = "long_only")

minvar <- add.objective(portfolio = port_l, type = "risk", name = "var")
opt_minvar <- optimize.portfolio(R=stock.returns.timeseries_xts, portfolio = minvar, optimize_method = "ROI", trace = TRUE)
...
```
Maximize mean return with ROI (exactly the same just maximizing the mean return)
```{r}
maxret <-add.objective(portfolio=port_l, type="return", name="mean")
opt_maxret <- optimize.portfolio(R=stock.returns.timeseries_xts, portfolio=maxret,optimize_method="ROI", trace=TRUE)
...
opt_qu <- optimize.portfolio(R=stock.returns.timeseries_xts, portfolio=qu, optimize_method="ROI", trace=TRUE)
...
chart.EfficientFrontierOverlay(R=stock.returns.timeseries_xts, portfolio_list=portf.list, type="mean-StdDev", match.col="StdDev", legend.loc="topleft", legend.labels=legend.labels, cex.legend=0.6, labels.assets=FALSE, pch.assets=1)
```
...
chart.EfficientFrontierOverlay(R=stock.returns.timeseries_xts, portfolio_list=portf.list.c, type="mean-StdDev", match.col="StdDev", legend.loc="topleft", legend.labels=legend.labels, cex.legend=0.6, labels.assets=FALSE, pch.assets=1)
```
...
```{r}
Eurostoxx_correct <- read_xlsx("eurostoxx.new.xlsx")
Eurostoxx_correct
View(Eurostoxx_correct)
Iboxx_correct <- read_xlsx("iboxx.new.xlsx")
Iboxx_correct
View(Iboxx_correct)
```
...
Get the necessary input parameters (mu, sigma, please using variables, I don't want to see manual numbers in your code) and calculate the Minimum-Variance-Portfolio (manually in R). Then do it using the `portfolioAnalytics`-package.
...
Now assume a risk-free rate of 0 and calculate the Tangency-Portfolio manually and with the `portfolioAnalytics`-package. What is the slope of the CAL? Plot a mu-sigma-diagram including all relevant information. What are your portfolio weights and weighted returns? Additionally allow for shortselling and check for changes.
