...
```{r load_packs}
pacman::p_load(tidyverse,tidyquant,FFdownload,PortfolioAnalytics,tsibble,matrixcalc,Matrix,timetk,dplyr,devtools,ggrepel,readxl,ROI)
```
%>% to pipe
...
```{r}
portf_minvar_tp <- portfolio.spec(assets = colnames(returns_index_final_xts), category_labels = labels)

portf_minvar_tp <- add.constraint(portfolio=portf_minvar_tp, type="full_investment")
portf_minvar_tp <- add.constraint(portfolio=portf_minvar_tp, type="box", min=-1, max=2)

minvartp <- add.objective(portfolio=portf_minvar_tp, type="risk", name="var")
minvartp <- add.objective(portfolio=portf_minvar_tp, type="return", name="mean")

opt_minvar_tp <- optimize.portfolio(R=returns_index_final_xts, portfolio=minvartp, optimize_method="ROI", trace=TRUE, maxSR=TRUE)
print(opt_minvar_tp)
```
...
slope of the CAL (Capital asset line) would be the Sharpratio = -0.0268
...
```{r}
mean_tangencyportfolio/(1*sd_tangencyportfolio^2)
```

```{r}
mean_tangencyportfolio/(2*sd_tangencyportfolio^2)
```

```{r}
mean_tangencyportfolio/(3*sd_tangencyportfolio^2)
```
